################################################################################
#                                                                              #
#       Plot Akima's bivariate interpolation (surface fitting) of growth       #
...
n.pred <- as.numeric(as.character(para[29,])) # Default: 1000
if (para[30,] == "NULL") { contour.levels <- NULL } else { contour.levels <- seq(from = as.numeric(as.character(para[30,])), to = as.numeric(as.character(para[31,])), by = as.numeric(as.character(para[32,]))) }
leafShapePredSurf <- as.logical(para[33,]) # Default: F
...
for (my_img in 1:(nrow(ProcessedImages)-1))
  ...
  arg <- list(InfoVertices = InfoVertices, Vertices = Vertices, Cells = Cells, Divisions = Divisions,
              ...
              alignToPetioleLaminaBoundary = alignToPetioleLaminaBoundary,
              Image1 = Image1, Image2 = Image2, leafShapePredSurf = leafShapePredSurf,
